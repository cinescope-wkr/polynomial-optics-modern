...
const TruncPoly <scalar, num_vars+1> TruncPoly _SN::lerp_with(const TruncPoly _SN &poly2, const scalar position1, const scalar position2) const {
  ...
  const scalar denom = (position2 - position1);
  ...
  for (int k = 0; k < (int)poly1.terms.size(); ++k) {
    ...
    // poly1 * (position2 - t) / (position2 - position1)
    result.terms[j].coefficient = c1 * (position2 / denom);
    ...
    result.terms[num_terms + j].coefficient = -c1 / denom;
  ...
  for (int k = 0; k < (int)poly2.terms.size(); ++k) {
    ...
    // poly2 * (t - position1) / (position2 - position1)
    result.terms[j].coefficient = -c2 * (position1 / denom);
    for (int ie = 0; ie < num_vars; ++ie)
    ...
    // Copy term
    result.terms[num_terms + j] = result.terms[j];
    result.terms[num_terms + j].coefficient = c2 / denom;
    result.terms[num_terms + j].exponents[num_vars] = 1;
    ++j;
  }
...
const TruncPoly <scalar, num_vars+1> TruncPoly _SN::querp_with(const TruncPoly _SN &poly2, const TruncPoly _SN &poly3, const scalar pos1, const scalar pos2, const scalar pos3) const {
  ...
  // Collect union of exponent vectors across all three polynomials.
  std::map<std::array<echar, num_vars>, std::array<scalar, 3>> coeffs_by_exp;

  auto add_terms = [&](const TruncPoly _SN& poly, int idx) {
    for (int i = 0; i < (int)poly.terms.size(); ++i) {
      std::array<echar, num_vars> key{};
      for (int v = 0; v < num_vars; ++v) key[v] = poly.terms[i].exponents[v];
      coeffs_by_exp[key][idx] += poly.terms[i].coefficient;
    }
  };
  ...
  const double x1 = (double)pos2;
  const double x2 = (double)pos1;
  const double x3 = (double)pos3;
  const double denom = (x1 - x2) * (x1 - x3) * (x2 - x3);
  assert(denom != 0.0);

  result.terms.reserve(coeffs_by_exp.size() * 3);
  for (const auto& kv : coeffs_by_exp) {
    const auto& exps = kv.first;
    const auto& ys = kv.second;

    const double y1 = (double)ys[1];
    const double y2 = (double)ys[0];
    const double y3 = (double)ys[2];

    const double A = (x3 * (y2 - y1) + x2 * (y1 - y3) + x1 * (y3 - y2)) / denom;
    const double B = (x3 * x3 * (y1 - y2)
                      + x2 * x2 * (y3 - y1)
                      + x1 * x1 * (y2 - y3)) / denom;
    const double C = (x2 * x3 * (x2 - x3) * y1
                      + x3 * x1 * (x3 - x1) * y2
                      + x1 * x2 * (x1 - x2) * y3) / denom;

    PolyTerm<scalar, num_vars + 1> termA((scalar)A);
    PolyTerm<scalar, num_vars + 1> termB((scalar)B);
    PolyTerm<scalar, num_vars + 1> termC((scalar)C);

    for (int v = 0; v < num_vars; ++v) {
      termA.exponents[v] = exps[v];
      termB.exponents[v] = exps[v];
      termC.exponents[v] = exps[v];
    }
    termA.exponents[num_vars] = 2;
    termB.exponents[num_vars] = 1;
    termC.exponents[num_vars] = 0;

    result.terms.push_back(termA);
    result.terms.push_back(termB);
    result.terms.push_back(termC);
  }

  if (poly1.trunc_degree >= 0 && poly2.trunc_degree >= 0 && poly3.trunc_degree >= 0)
    result %= max(max(poly1.trunc_degree, poly2.trunc_degree), poly3.trunc_degree) + 2;
  else result %= -1;
...
const TruncPoly <scalar, num_vars-1> TruncPoly _SN::bake_input_variable(int varIdx, scalar value) const {
  static_assert(num_vars > 1, "bake_input_variable requires num_vars > 1");
  assert(varIdx >= 0 && varIdx < num_vars);

  TruncPoly<scalar, num_vars - 1> result;
  result.trunc_degree = trunc_degree;
  result.terms.resize(terms.size());

  // Copy terms over, removing exponent column varIdx and absorbing it into the coefficient.
  for (int i = 0; i < (int)terms.size(); ++i) {
    result.terms[i].coefficient = terms[i].coefficient * intpow(value, terms[i].exponents[varIdx]);
    int j = 0;
    for (int v = 0; v < num_vars; ++v) {
      if (v == varIdx) continue;
      result.terms[i].exponents[j++] = terms[i].exponents[v];
    }
  }
...
scalar TruncPolySystem _SIO::evaluate_line(const scalar x0[num_vars_in], int i){
  ...
  return equations[i].evaluate(x0);
}
